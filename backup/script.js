...
// Show terms modal on page load if not already accepted
document.addEventListener('DOMContentLoaded', () => {
    if (!termsAccepted) {
        showTermsModal();
    } else {
        // Hide site blocker if terms were previously accepted
        siteBlocker.classList.add('hidden');
    }
});
...
document.querySelector('.modal-body').addEventListener('wheel', (event) => {
    event.stopPropagation();
});

// Toggle accept button state based on checkbox
termsCheckbox.addEventListener('change', () => {
    acceptTerms.disabled = !termsCheckbox.checked;
...
// Close modal functions - only allowed after accepting terms
function closeTermsModal() {
    termsModal.classList.remove('active');
    document.body.style.overflow = '';
}

// Close when X button is clicked (hidden by default for initial load)
closeModal.addEventListener('click', closeTermsModal);

// Accept terms and allow access to the site
acceptTerms.addEventListener('click', () => {
    if (termsCheckbox.checked) {
        ...
        // Hide the site blocker
        siteBlocker.classList.add('hidden');
        
        // Enable the close button for future openings
        closeModal.style.display = 'block';
    }
});

// Don't allow closing the modal by clicking outside initially
termsModal.addEventListener('click', (event) => {
    if (event.target === termsModal && termsAccepted) {
        closeTermsModal();
    }
});

// Only allow escape key to close modal if terms were previously accepted
document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && termsModal.classList.contains('active') && termsAccepted) {
        closeTermsModal();
    }
});
