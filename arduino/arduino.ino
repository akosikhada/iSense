...
void checkLCD() {
  if (lcdUpdateNeeded) {
    lcd.clear(); // Clear the entire display first to avoid character artifacts
    lcd.setCursor(0, 0);
    lcd.print(lcdLine1);
    lcd.setCursor(0, 1);
    lcd.print(lcdLine2);
...
void checkAlignment() {
  ...
  if (currentAlignmentStatus != "OK" && currentAlignmentStatus != "NOT_OCCUPIED" && !buzzerActive) {
    ...
    updateLCD("    WELCOME!    ", "  Slot Left: " + String(Slot) + "  ");
  }
      ...
      if (Slot > 0) {
        ...
        if (!slot2Occupied) {
          ...
          Serial.flush();
                ...
                // Send updated slot count immediately        Serial.print("SLOTS:");        Serial.println(Slot);        Serial.flush();        delay(2000);  // Wait for car to pass        servoEntry.write(100);  // Close entry gate                // Make sure to pad with spaces to clear any previous digits        String slotDisplay = "  Slot Left: " + String(Slot) + "     ";        updateLCD("    WELCOME!    ", slotDisplay);
      } else {
